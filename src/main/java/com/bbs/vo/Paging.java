...
public class Paging {
	...
	private int before;		// 이전 버튼을 눌렀을 때 이동할 페이지 번호
	private int forward;	// 다음 버튼을 눌렀을 때 이동할 페이지 번호
	...
	public Paging(int pageNumber, int count) {
		...
		int section;
		if(pageNumber % PAGING_CNT == 0) {
			section = pageNumber / PAGING_CNT;
		} else section = pageNumber / PAGING_CNT + 1;
		
		next = count > section * (PAGING_CNT * BOARD_CNT);
		
		minPage = 1 + PAGING_CNT * (section - 1);
